...
**更正一处我先前的错误判断（据实记录）**：我在阶段报告里写"设备详情抽屉的流转历史仍为**表格**形态"——**这是错的**。实际代码（`EquipmentPage` 抽屉）**本来就是 antd `<Timeline>`**。我当时没有读那段代码就下了结论。教训：报告结论必须以**读过的代码**为依据 —— `engineering-persona` 的「不猜测」同样适用于自述。

**Phase 5 追加迭代（2026-09-12，用户批准 ①）** —— 读代码后确认时间线**真实存在的两个问题**并修复：

1. **静默截断**：`items={txns.slice(0, 40)...}` 只渲染最近 40 条，而卡片标题写的是「共 N 条」。真实设备流转条数很小（全库 2481 条 / 2148 台，均值约 1.16），40 条几乎不会触发；但**一旦触发就是"标题说 N 条、只画 40 条"的不实呈现**。已去掉截断、渲染全部（后端 `limit=200` 已是天然上界）。
2. **圆点只有两档颜色**（报废红、其余蓝）：改为按**目标状态**取语义色 `statusFillVar(t.to_status)`，报废仍单独用危险色强调。时间线因此能一眼看出「每次流转把设备带到了哪个状态」，且与 Phase 4 的颜色单一来源一致。

**安全门槛统一（用户批准 ②）** —— 「清空重导」由「点一次确认」提升为与「恢复备份」同级：**必须手动输入确认词 `RESET`** 才能执行（确认按钮在输入正确前保持禁用）。

- **只提高前端确认强度，后端门槛未动**：`/api/import/reset` 仍要求 `confirm:true`，既有 `TestImportResetAndReimport` 继续守护后端。
- 顺带把「恢复备份」的确认输入框由**浏览器原生 `<input>`** 换成 antd `<Input>`，与全站控件一致（也避免原生控件在深浅两模式下的样式/对比度不确定）。
- 验证：`tsc --noEmit` ✅、`vite build` ✅（3339 模块）、约定检查脚本 **6 组全 PASS** ✅；`slice(0, 40)` 残留扫描为 0；确认词门槛源码核对在位。**确认词交互与时间线观感待用户实机验收**。
