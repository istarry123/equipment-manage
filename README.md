...
> **v1.2（2026-09-09）**：新增「设备流转情况」Excel 导出 —— 三工作表（当前流转情况 / 流转历史 / 统计汇总），支持班组/状态/外借公司筛选 + 数据范围选择；编号按文本导出（`001` 不丢零、中文编号保留）、重复编号用 `6041（1）（2）…` 区分；严格以 `equipment.id` 关联历史、以数据库为唯一数据源（不重读原始 Excel）；每次导出记审计。入口：设备台账页「导出流转情况」（原「导出设备台账」保留）。
>
...
| **v1.2.0** | **设备流转情况导出**：三工作表（当前流转情况/流转历史/统计汇总）、班组/状态/外借公司筛选、数据范围选择、编号文本导出、审计留痕 |
| **v1.3.0** | **导入安全加固 + 外借明细补录**：导入前模板表头校验（拒绝列布局不符的文件）、外借明细通道（到达时间/外借方/数量/设备编号）解析→匹配→按外借方分批补录（外借N 标签、同号多台按序分配、单事务写入）→数据对账 |
| **v1.4.0** | **界面改版（不换框架）**：设计令牌落地（深色默认 + 一键切换浅色，色值单一来源，状态色过 WCAG AA）、页面路由化（URL 可直达/刷新不丢页）、Dashboard 改版（KPI Hero + 卡片网格，**数值零变化**）、列表页统一（编号等宽、吸顶表头、筛选空态）、危险操作受控（统一后果提示；清空重导与恢复同门槛需输入确认词），另新增安全不变量测试 |
