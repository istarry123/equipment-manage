...
> **功能增量（v1.0.0 之后）**：新增「班组设备」视图 —— 按「班组 → 设备类别 → 设备」三级查看各班组当前使用设备；未分配设备（在库/外借/维修，`current_team_id` 为空）独立分组便于异常管理；设备编号可点击进入既有设备详情；与设备台账、Dashboard 统计同源（`equipment.current_team_id/status/category`），不含外借误入班组。
