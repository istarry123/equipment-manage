...
- **v1.3 导入安全加固（2026-09-11，决策 19）**：
  ...
  - **Phase 3 待办（本次 STOP 等验收）**：匹配 / 预览 / 确认 —— 同号多台按用户选定「方案 A」（文件中补 `设备名称/型号` 占位）定位、未匹配 `0430701` 新建设备；随后 Phase 4 单事务写入（BORROWED + borrow_record + flow_record + 审计 + 对账）。
  - 构建：`scripts/build-release.ps1` 产出 `release/equipment/`（含 Phase 1+2；**未覆盖根目录 `equipment.exe`**，按用户要求）。
