...
- v1.1 Phase 8（Frontend display_no）要点（§三十四~§三十七）：后端补齐 display_no 下发（班组视图/Dashboard/外借/导出，同组计数统一 `service.EquipmentSelectNoGrp`）；搜索支持 display_no（剥离（n）后缀匹配）；前端台账/班组/Dashboard/外借/导入结果统一显示 display_no；rowKey 审计均为 equipment.id。
