...
- v1.1 Phase 11（Reconciliation 对账）要点（§四十七）：`importer.Reconcile(db,res)` 以 source_hash 关联批次，Excel 可导入(source_key 全集) ↔ DB 该批次设备逐台比对：缺失/多出逐台列出、无编号/同号多台双侧计数、历史借出未匹配；全等 PASS 否则 FAIL；`POST /api/import/reconcile {parse_id}`（run 后保留会话供对账）；前端 ImportPage 导入后对账卡片；真实文件 e2e 2147==2147 PASS。
