...
- **v1.4 界面改版（2026-09-12，决策 20；方案 `docs/ui-redesign-plan.md`、令牌 `docs/ui-design-tokens.md`）**：
  ...
  - **Phase 5 关键流程与危险操作 ✅ 本次** —— ①**新增安全不变量测试** `internal/api/safety_test.go`（4 用例，锁住此前**未被任何测试覆盖**的危险操作性质）：`TestNoEquipmentDeleteRoute`（4 条 DELETE 路径全 404，设备禁止物理删除）、`TestEquipmentNumberNotEditableViaPut`（PUT **夹带 `equipment_no` 也被忽略**，决策 13 由"请求结构体不含该字段"结构强制，用测试锁住）、`TestRestoreRequiresConfirm`（缺 filename / confirm=false → 400 且统一错误体）、`TestScrappedIsTerminalViaAPI`（报废后 4 种动作全 400，且**状态未改写、流转历史条数未增加**）。②**新增 `web/src/DangerNotice.tsx`** 统一危险操作文案层级（做什么 → 后果与可回退性），应用于**报废 / 清空重导 / 恢复备份**；组件注释明确边界：**只统一呈现，不改任何确认门槛**。③**修掉流转弹窗两个真实缺口**：报废**无终态后果说明**（已补）、报废确认按钮**非危险色**（已加 `danger`）。`ImportDetailPage` 补录确认的后果说明本就比通用模板更细（含范围清单），**不做降级**替换。
    ...
    - 验证：`tsc --noEmit` ✅、`vite build` ✅（3339 模块）、`go build ./...` ✅、`go test ./... -count=1` **全绿**（含 4 个新安全用例）✅、约定检查脚本 6 组全 PASS ✅；实机安全实测 4 项符合预期；冒烟写入的预览数据副本已**还原为与生产库字节一致**（`B50615C1…`），生产库全程未变。**待用户实机验收**：`DangerNotice` 弹窗观感、报废确认按钮危险色显示。
  - **Phase 6 交付与回归 ✅ 本次（tag `v1.4.0`）** —— ①**版本统一 v1.4.0**（`cmd/equipment/main.go` + `web/src/App.tsx` + `README.md` 版本表新增行）。②**`docs/user-guide.md`** 标题升 v1.4.0，新增「界面外观与导航（v1.4）」小节（深色/浅色切换与记忆、页面地址可直达 + F5 刷新不丢页、侧栏折叠、编号等宽、表头吸顶、设备详情时间线），「清空重导」两处补 **需输入 RESET 确认**，并写明「以上均为外观与导航改动，业务行为完全未变」。③`docs/roadmap.md` 新增 §16 v1.4 完成记录。④**发布构建** `scripts/build-release.ps1` 成功，产物 `release/equipment/`（exe 28.4 MB）。
    - **本阶段发现并修复两处真实交付阻塞缺陷**：(a) **发布脚本被 npm 的警告打断** —— 脚本 `$ErrorActionPreference='Stop'`，而 PS 5.1 把原生命令的 stderr 普通输出当终止性错误，`npm run build` 每次都写 chunk 体积警告 → 第 1 步即抛 RemoteException（已单独复现：EAP=Stop 抛异常 / EAP=Continue 则 `npm exit=0`）；修复为 `Invoke-Native` 辅助函数（临时放宽 EAP、照常打印、**改按 `$LASTEXITCODE` 判定**）；修复中又踩到「**`$Args` 是 PowerShell 自动变量**」导致参数根本传不进去（npm 打印用法帮助）→ 改名 `$ArgList` 并注明。(b) **交付目录数据库是三天前的旧库** —— `release/equipment/equipment.db` 为 1,380,352 字节 / `6F7660F8…` / 文件时间 2026-09-09 12:27（当前生产库为 1,503,232 / `B50615C1…`），自检确认其数据为 **2147 台（在库 2028 / 外借 119）**，**缺失 v1.3 全部外借明细补录**；`AGENTS.md` 曾记「已用生产库覆盖」与该产物时间戳/哈希不符 —— **以产物实测为准**；已用生产库覆盖交付库（哈希一致），旧库移出交付目录留存 `.tmp/release-db-stale-20260909.db` 作证据。
    - **产物自检**（从交付目录副本启动，端口 8092，`EQ_NO_BROWSER=1`）：`version=1.4.0` ✅、内嵌前端 `index-4bdcd131.js`（= 本次发布产物）✅、数据 `total=2148` / 在库 **1815** / 外借 **333** / 外借单 **333** ✅。
    - **回归**：`go vet ./...` exit 0 ✅、`go test ./... -count=1` 全绿（含 4 个安全用例）✅、前端约定脚本 6 组全 PASS ✅、`tsc`/`vite build` ✅、无测试硬编码旧版本号 ✅。
    - **未完成（须在目标电脑做）**：Win10/11 全流程走查、Win7 SP1 实机回归（Chrome109/FF115）、Chrome109 离线包放入 `release/equipment/install/`（本环境无法访问 dl.google.com）。**另需注意（未擅自处理）**：`release/equipment/backup/` 内有 3 个 128 KB 的历史测试备份，会随目录交付，建议发布前清空或由用户决定。
- 最终验收待办（目标电脑）：① Win10/11 全流程走查；② Win7 SP1 实机回归（需 Chrome109/FF115）；③ Chrome109 离线包放入 install/；④ v1.1 全流程（状态推导→display_no→Preview/Review→清空重导→对账→回归→文档）验收；⑤ v1.2 导出流程走查；⑥ v1.3 Phase 1 模板校验实测（上传非模板文件应被拒绝并提示）；⑦ **v1.4 全流程走查**（深色/浅色切换与记忆、地址栏直达各页 + F5 不丢页、Dashboard 观感、列表页编号等宽与吸顶表头、设备详情时间线、报废后果提示、清空重导需输入 RESET）。
