...
- v1.1 Phase 10（真实 Excel 终验 / 清空重导）要点（决策18⑤ + §四十六）：`POST /api/import/reset`（confirm 二次确认 → 自动备份 → `service.ClearImportData` 单事务清空 equipment/flow_record/borrow_record/import_batch → `IMPORT_RESET` audit；保留字典/settings/audit 历史；空库幂等）；前端 ImportPage 清空重导危险入口；e2e 测试：非空库 reset → parse → REVIEW 全清点 → 全量导入 2147 台。
