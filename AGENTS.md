...
- **v1.3 导入安全加固（2026-09-11，决策 19）**：
  ...
  - **后续（待用户验收 Phase 4 后）**：① 正式在执行前备份生产库并由用户在页面点「确认补录」；② 可选 Phase 5：本通道的正式对账（Excel ↔ DB 逐台比对）与版本号 v1.3.0 收尾 + tag；③ 若需要，把「外借N 标签」显示到外借管理页。
  - 构建：`scripts/build-release.ps1` 产出 `release/equipment/`（含 Phase 1–4；**未覆盖根目录 `equipment.exe`**，按用户要求）。
