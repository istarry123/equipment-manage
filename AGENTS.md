...
- 进度：**Phase 1 已完成（2026-09-08，待验收）**：骨架落地 —— Go1.20.14 工具链可编译、SQLite 版本化迁移（8 表）、Gin REST(/api/health)、React+AntD 骨架（go:embed 内嵌、target chrome109）、`equipment.exe` 冒烟通过、单测全绿（tag v0.2.0-phase1）。决策基线 15 项。待确认清单进度：**W-2/W-5 已答复（决策 15）**；Blockers 剩余 **W-1（类别标注）、W-3/W-4（编号重复与唯一粒度）**，Phase 2（Excel 导入）启动前答复。**Phase 2 未启动**。
