...
- **v1.3 导入安全加固（2026-09-11，决策 19）**：
  ...
  - **Phase 4 待办（本次 STOP 等验收）**：前端人工选择交互（同号多台候选、未匹配新建/跳过）+ `POST /api/import/borrow-detail/run` 单事务写入（外借方建档 → BORROWED + borrow_record + flow_record（含历史日期）→ 审计 → 对账报告）。**开工前需用户确认**：9 条「文件出现 1 次 vs 库中 2 台」的选择口径、`0430701` 新建用的名称/型号。
  - 构建：`scripts/build-release.ps1` 产出 `release/equipment/`（含 Phase 1–3；**未覆盖根目录 `equipment.exe`**，按用户要求）。
